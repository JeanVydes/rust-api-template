...
use crate::primitives::{GenericResponse, User};
...
pub async fn request_credentials(
    ...
    let users_collection = state.mongo_db.collection("users");
    let user: User = match users_collection
        .find_one(
            doc! {"$or":
                [
                    {"username": &payload.username_or_email.to_lowercase()},
        ...
        Ok(user) => match user {
            ...
            None => {
                return (
                    ...
                    Json(GenericResponse {
                        message: String::from("username or email not associated with any account"),
                        data: json!({}),
                        exited_code: 0,
                    }),
                )
